...
export function FinalLetter({ paragraphs, onComplete }: FinalLetterProps) {
  ...
  return (
    <div className="flex w-full flex-col items-center gap-10 sm:gap-12">
      <div className="selectable-text safe-px mx-auto w-full max-w-[700px] px-4 sm:px-6 md:px-8">
        <div className="flex flex-col gap-6 sm:gap-7 md:gap-9">
          {paragraphs.slice(0, visibleCount).map((paragraph, index) => (
            <motion.p
              ...
              className="font-[Caveat] text-pretty text-[clamp(1.2rem,3.5vw+0.5rem,1.6rem)] leading-[1.75] text-text/90 sm:leading-[1.8]"
            >
              {paragraph}
            </motion.p>
          ))}
